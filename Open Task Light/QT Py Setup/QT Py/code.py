...
# Onboard NeoPixel is on pin board.NEOPIXEL
pixel = neopixel.NeoPixel(board.NEOPIXEL, 1, brightness=0.3)  # adjust brightness 0.0–1.0

# this is just for basic boot testing you can comment it out or delete it or leave it.
pixel[0] = (0, 255, 0)  # Solid green (R, G, B)
...
DEFAULT_POWER_OUTPUT = 100 # out of 100. used for global max power ceiling
POWER_OUTPUT_LIMIT = DEFAULT_POWER_OUTPUT  # used to adjust max output based on temperature
DAC_BRIGHTNESS_FLOOR = 55000# use this to ensure low brightness settings are visible/optimal. Also allows you to manual adjust the brightness of the lowest brightness level.
DAC_OFF_VALUE = DEFAULT_MAX_16_BIT_VALUE
BRIGHTNESS_RANGE = DAC_OFF_VALUE - DAC_BRIGHTNESS_FLOOR

# INITIALIZE THE RX PIN WHICH TURNS OFF THE IC SWITCH AFTER BOOT
...
############
# LAMP SETUP
_power = False  # False means lamp is off by default (don't change this)
...
temperature_sample_rate = 3 #time in seconds to poll temperature sensor, not much benefit to reducing this (outside of testing)
...
def check_temperature():
    ...
    # Only check temperature every X seconds
